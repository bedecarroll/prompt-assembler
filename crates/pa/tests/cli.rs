...
fn base_command() -> Command {
    // Canonicalize so the binary is still found when tests set current_dir
    // (Bazel provides a runfiles-relative path; cargo's is already absolute).
    let bin = std::path::PathBuf::from(assert_cmd::cargo::cargo_bin!("pa"));
